...
- 这 6 个按钮后续会承载消息、互动、礼物、关系、生成、设置等功能入口。
- 发送消息不再使用单独的第 7 个入口，直接复用现有 6 个按钮之一。
...
5. 发送消息功能直接复用 `act-typing`/敲电脑按钮打开消息输入面板。
...
当前 6 个原动作按钮保留视觉和弹出方式，但语义从动作改为功能；发送消息直接复用 `act-typing`/敲电脑按钮：

| 当前入口 | 本轮行为 | 后续方向 |
| --- | --- | --- |
| 撒娇/卖萌 | 占位反馈或随机播放一个 motion | 情绪互动、亲密度 |
| 敲电脑 | 打开消息输入面板 | 发消息、留言、后续聊天入口 |
| 打招呼 | 占位反馈或随机播放一个 motion | 远程招呼 |
| 抱抱 | 占位反馈或随机播放一个 motion | 远程互动 |
| 嘟嘴/生气 | 占位反馈或随机播放一个 motion | 情绪表达 |

除了复用为发送消息的 `act-typing` 外，本轮不需要让原动作按钮强制播放同名动画。其他按钮可以先统一触发一个轻量反馈，例如气泡“功能开发中”，或随机播放动作池中的一个 motion。具体实现计划阶段再定。
...
5. 单击弹出的 6 个原动作按钮仍显示，不新增第 7 个发送消息入口。
6. 非发送消息按钮不再要求资源包存在同名动作。
7. `act-typing`/敲电脑按钮触发发送消息功能并保持可用。
